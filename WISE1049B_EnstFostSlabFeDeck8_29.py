...
# Give the run name
runname = "WISE1049B_EnstFostSlabFeDeck8_29"
...
obspec = np.asfortranarray(np.loadtxt("WISE1049B_Median.txt",dtype='d',unpack='true'))
...
if (fresh == 0):
    ...
    p0[:,0] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 3.6 # H2O
    p0[:,1] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 5.0 # CH4
    p0[:,2] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 3.5 # CO 
    p0[:,3] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 7.0 # CO2
    p0[:,4] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 6.0 # NH3   
    p0[:,5] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 10.0 # FeH
    p0[:,6] = (0.5*np.random.randn(nwalkers).reshape(nwalkers)) - 4.5 # Na+K
   ...
   # ------ Cloud Parameters ------
    ...
    # Slab cloud 2 params
    p0[:,17] = np.random.rand(nwalkers).reshape(nwalkers) # optical depth
    ...
    # e.g grey cloud or power law cloud, or no cloud, or deck cloud
    # this example is a "real" cloud with Hansen a and b parameters
    p0[:,22] = np.random.randn(nwalkers).reshape(nwalkers) # cloud top pressure
    ...
    p0[:,26] = 1200. + (100.* np.random.randn(nwalkers).reshape(nwalkers))
    p0[:,27] = (0.1 * np.random.randn(nwalkers).reshape(nwalkers)) + 1.25
    p0[:,28] = (0.1 * np.random.randn(nwalkers).reshape(nwalkers)) - 1.2
    p0[:,29] = 115. + (100. * np.random.randn(nwalkers).reshape(nwalkers))
    p0[:,30] = 1130. + (100. * np.random.randn(nwalkers).reshape(nwalkers))
    p0[:,31] = 765. + (100. * np.random.randn(nwalkers).reshape(nwalkers))
