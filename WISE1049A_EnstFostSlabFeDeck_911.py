...
if (fresh == 0):
    ...
    p0[:,0] = (0.05*np.random.randn(nwalkers).reshape(nwalkers)) - 3.6 # H2O
    p0[:,1] = (0.05*np.random.randn(nwalkers).reshape(nwalkers)) - 5.15 # CH4
    p0[:,2] = (0.05*np.random.randn(nwalkers).reshape(nwalkers)) - 3.5 # CO 
    p0[:,3] = (0.05*np.random.randn(nwalkers).reshape(nwalkers)) - 4.25 # CO2
    p0[:,4] = (0.05*np.random.randn(nwalkers).reshape(nwalkers)) - 6.75 # NH3   
    p0[:,5] = (0.05*np.random.randn(nwalkers).reshape(nwalkers)) - 10.0 # FeH
    p0[:,6] = (0.05*np.random.randn(nwalkers).reshape(nwalkers)) - 5.5 # Na+K
    p0[:,7] = 0.05*np.random.randn(nwalkers).reshape(nwalkers) + 4.75  # gravity
   ...
   # ------ Cloud Parameters ------
    ...
    p0[:,12] = np.abs(np.random.rand(nwalkers).reshape(nwalkers)) # optical depth
    p0[:,13] = -2.0 + 0.05 * np.random.randn(nwalkers).reshape(nwalkers) # cloud top pressure
    p0[:,14] = np.random.rand(nwalkers).reshape(nwalkers) # cloud thickness in pressure
    p0[:,15] = np.abs(0.1 + 0.05*np.random.randn(nwalkers).reshape(nwalkers)) # Hansen a if "real" cloud or single scattering albedo between 0 and 1 (np.rand=uniform distribution) for 89/99 cloud
    p0[:,16] = 0.05*np.random.rand(nwalkers).reshape(nwalkers) # Hansen b for "real" cloud or Power law for 89/99 cloud
    
    # Slab cloud 2 params
    p0[:,17] = np.abs(np.random.rand(nwalkers).reshape(nwalkers)) # optical depth
    p0[:,18] = -2.0 + 0.05 * np.random.randn(nwalkers).reshape(nwalkers) # cloud top pressure
    p0[:,19] = np.random.rand(nwalkers).reshape(nwalkers) # cloud thickness in pressure
    p0[:,20] = np.abs(-0.1 + 0.05*np.random.randn(nwalkers).reshape(nwalkers)) # Hansen a if "real" cloud or single scattering albedo between 0 and 1 (np.rand=uniform distribution) for 89/99 cloud
    p0[:,21] = 0.05*np.random.rand(nwalkers).reshape(nwalkers) # Hansen b for "real" cloud or Power law for 89/99 cloud
   
    # Deck cloud params
    # These parameters should be commented out or adjusted for
    # e.g grey cloud or power law cloud, or no cloud, or deck cloud
    # this example is a "real" cloud with Hansen a and b parameters
    p0[:,22] = 0.2 + 0.05 * np.random.randn(nwalkers).reshape(nwalkers) # cloud top pressure
    p0[:,23] = np.random.rand(nwalkers).reshape(nwalkers) # cloud thickness in pressure
    p0[:,24] = 0.2 + 0.05 * np.random.randn(nwalkers).reshape(nwalkers) # Hansen a if "real" cloud or single scattering albedo between 0 and 1 (np.rand=uniform distribution) for 89/99 cloud
    p0[:,25] = np.abs(0.1 + 0.05*np.random.randn(nwalkers).reshape(nwalkers)) # Hansen b for "real" cloud or Power law for 89/99 cloud
    ...
    #     p0[:,ndim-13 + i] = (BTprof[i] - 50.) + (50 * np.random.randn(nwalkers).reshape(nwalkers))
    ...
    p0[:,26] = 1000. + (10.* np.random.randn(nwalkers).reshape(nwalkers))
    p0[:,27] = (0.1 * np.random.randn(nwalkers).reshape(nwalkers)) + 1.0
    p0[:,28] = (0.1 * np.random.randn(nwalkers).reshape(nwalkers)) - 2.4
    p0[:,29] = 620. + (10. * np.random.randn(nwalkers).reshape(nwalkers))
    p0[:,30] = 680. + (10. * np.random.randn(nwalkers).reshape(nwalkers))
    p0[:,31] = 715. + (10. * np.random.randn(nwalkers).reshape(nwalkers))
